...
class GridWorldEnv(gym.Env):
    ...
    def __init__(
        ...
        rewarding_machines = [
            ...
            Environment_data.create_third_individual_rm(),
        ]

        # save the RM draw and create the agents
        for agent_idx, agent_events in enumerate([Environment_data.agent_1_events,
                                                  Environment_data.agent_2_events,
                                                  Environment_data.agent_3_events,
                                                  ]):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                'agent_3': spaces.Box(0, self.size - 1, shape=(2,), dtype=int),
            }
        )
    ...
    def _get_info(
        ...
        return {
            ...
            'agent_3': [np.linalg.norm(self.agents[2].position - self._targets_location[0], ord=1)],
        }
    ...
    def reset(
        ...
        for agent_idx, agent in enumerate(self.agents):
            ...
            self.agents[agent_idx].position = Environment_data.agents_initial_location[agent_idx]
            # select new intention
            if self.training:
        ...
        self.agents[2].events = Environment_data.agent_3_events
    ...
    def step(
        ...
        for agent_idx, action in enumerate(list(actions.values())):
            ...
            # check for push button to open door
                ...
                if (self.open_door(agent_idx, 0, 0, 'red', openers) or
                    ...
                    self.open_door(agent_idx, 2, 0, 'green', openers)):
            ...
            # check for open pocket door
                ...
                if self._doors_flag[1] == 1:
                    if self.open_pocket_door(agent_idx, 2, 'door_3'):
                        reward[agent_idx] = Environment_data.subtask_reward

                if self._doors_flag[1] == 0:
                    if self.open_pocket_door(agent_idx, 3, 'door_4'):
                        reward[agent_idx] = Environment_data.subtask_reward

            # check of final position of the agents
            if (agent_idx == 0 and self.reach_target(agent_idx, 0, 'target_1') or
                agent_idx == 2 and self.reach_target(agent_idx, 2, 'target_3')):
                reward[agent_idx] = Environment_data.complete_reward

            if self._doors_flag[1] == 0:
                if (agent_idx == 1 and self.reach_target(agent_idx, 1, 'target_2') and
                        self.agents[agent_idx].get_selected_task() == 'target_2'):
                    reward[agent_idx] = Environment_data.complete_reward
            ...
            if (
                ...
                for transition in temp_goal:
                    for atom in str(sympify(transition[1])).replace(' ', '').replace('|', '&').split('&'):
                        if (not atom[0] == '~'
                                and not atom == 'target_1'
                                and not atom == 'target_2'
                                and not atom == 'target_3'
                        ):
                            atoms.append(atom)
                if atoms:
        ...
        if not self.training:
            terminated = [
                ...
                'target_3' not in self.next_event,
            ]
        else:
            terminated = [
                'target_1' not in self.next_event,
    ...
    def plot_trust(self, trust_list):
        ...
        ax7.title.set_text(Environment_data.events[6] + ' trust')
        ...
        ax7.set(xlabel='Epochs', ylabel='Trust')
        ...
        ax7.plot(trust_list[1][6], 'm', label='agent_2')
        ax7.plot(trust_list[2][6], 'c', label='agent_3')
        ax7.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3)
        ...
        ax1.title.set_text(Environment_data.events[7] + ' trust')
        ax2.title.set_text(Environment_data.events[8] + ' trust')
        ax2.title.set_text(Environment_data.events[9] + ' trust')
        ...
        ax1.plot(trust_list[0][7], 'y', label='agent_1')
        ax1.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3)

        ax2.plot(trust_list[1][8], 'm', label='agent_2')
        ax2.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3)

        ax3.plot(trust_list[2][9], 'g', label='agent_3')
        ax3.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3)
    ...
    def _render_frame(
        ...
        agent_green_img = pygame.transform.scale(
            pygame.image.load(Environment_data.agent_green_path), (pix_square_size, pix_square_size)
        )
        ...
        agent_four_img = pygame.transform.scale(
            pygame.image.load(Environment_data.agent_four_path), (pix_square_size, pix_square_size)
        )
        ...
        closed_door_4_img = pygame.transform.scale(
            pygame.image.load(Environment_data.closed_door_4_path), (pix_square_size, pix_square_size)
        )
        open_door_4_img = pygame.transform.scale(
            pygame.image.load(Environment_data.open_door_4_path), (pix_square_size, pix_square_size)
        )
        ...
        # draw pocket door 4
        if self._pocket_doors_flag[3] == 1:
            # Draw the closed doors
            canvas.blit(
                closed_door_4_img,
                pygame.Rect(
                    pix_square_size * self._pocket_doors_location[3],
                    (pix_square_size, pix_square_size),
                )
            )
        else:
